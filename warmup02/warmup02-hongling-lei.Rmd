...
```
1.1 The heaviest player is named __Zaza Pachulia__.

```{r}
college[height == '6-6']
```
1.2 The college of the player that has a height of 6-6 is __University of Arizona__.

```{r}
position[which.max(experience)]
```
1.3 The position of players with more years of experience is __C__.

```{r}
number[which.min(weight)]
```
1.4 The number of the lightest player is __0__.
...
```
1.6 __Four__ players have a weight larger than the average weight.

```{r}
sum(table(number[experience >= 9 & experience <=12]))
```
1.7 __Three__ players have between 9 and 12 years of experience (inclusive).

```{r}
mean(experience[position == 'SG'])
```
1.8 The mean years of experience of Shooting Guard players is __5.666667__

```{r}
median(weight[position != 'C'])
```
1.9 The median weight of those players with a position different of Center(C) is __212.5__

```{r}
quantile(experience[position == 'PF' | position == 'SG'])
```
1.10 The first quartile is __5__.
...
```
2.1 The number of the heaviest player is __27__.

```{r}
subset(gsw$position, gsw$experience == min(gsw$experience))
```
2.2 The position of the player with less experience is __SG__.

```{r}
library(plyr)
sum(table(subset(gsw$player, gsw$experience<8 | gsw$experience>11)))
```
2.3 __Seven__ players have less than 8 or more than 11 years of experience.

```{r}
quantile(subset(gsw$experience, gsw$position == 'PF' | gsw$position == 'SG'))
```
2.4 The third quartile is __10__.

```{r}
gsw$diff = (gsw$weight - mean(gsw$weight))
subset(gsw$player, gsw$diff == max(gsw$diff))
```
2.5 The player whose weight is furthest from the average weight (of all players) is named __Zaza Pachulia__.
...
class(hp)
```
3.1 The class of hp is __"list"__.

```{r}
length(hp)
```
3.2 __Six__ elements are in the list.

```{r}
length(hp$courses)
```
3.3 The length of courses is __3__.

```{r}
typeof(hp$age)
```
3.4 The data type of age is __integer__.

```{r}
typeof(hp$gpa)
```
3.5 The data type of gpa is __double__.

```{r}
NewVec <- c(hp$age, hp$gpa)
typeof(NewVec)
```
3.6 The type of the new vector is __double__.
