...
1.  The heaviest player is named **Zaza Pachulia**.
...
1.  The college of the player that has a height of 6-6 is **University of Arizona**.
...
1.  The position of players with more years of experience is **C**.
...
1.  The number of the lightest player is **0**.
...
1.  The frequencies of the position values are above.
...
1.  **Four** players have a weight larger than the average weight.
...
1.  **Three** players have between 9 and 12 years of experience (inclusive).
...
1.  The mean years of experience of Shooting Guard players is **5.666667**
...
1.  The median weight of those players with a position different of Center(C) is **212.5**
...
1.  The number of the heaviest player is **27**.
...
1.  The position of the player with less experience is **SG**.
...
1.  **Seven** players have less than 8 or more than 11 years of experience.
...
1.  The player whose weight is furthest from the average weight (of all players) is named **Zaza Pachulia**.
...
1.  **Six** elements are in the list.
...
1.  The data type of age is **integer**.
...
1.  The data type of gpa is **double**.
...
1.  Although "-2" is a character while 0 is a double, they are all transformed/coerced as double when R tries to compare them. Since -2 &gt; 0 is mathematically wrong, the running result is FALSE.
    To check that the FALSE isn't because we are comparing two different types of data, I run "2" &gt; 0 and the result is TRUE.

2.  By running (10 &lt;= 5) &gt;= 0 we get TRUE because R first run (10 &lt;= 5) and this part generates a FALSE, which equals 0 in number. Then R compares 0 with 0. Since 0 &gt;= 0, the final result is TRUE.
...
1.  They are different because of the order of calculation and coercion.
...
1.  Subsetting. length(lord) = 9, so *length(lord) + 1* = 10. Then the command becomes lord\[10\], but there are only 9 elements in it, so the result is NA because there's no such the 10th element.
...
1.  Subsetting with logical indices.
...
1.  Subsetting. *%in%* is a tool to search whether the vectors before and after *%in%* have any overlap. In this example, "voldemort" and "aeiou" have "e" and "o" in common, so the result displays all "e"s and "o"s in the vector lord; first appear, first display. So we get "o", "e", "o".
...
1.  (Cont.) Alternatively, we can think this way: R runs the inner command *lord %in% c('a', 'e', 'i', 'o', 'u')* first, and we get *FALSE TRUE FALSE FALSE TRUE FALSE TRUE FALSE FALSE*, then under the outer command *lord*, only the elements in *TRUE* positions are displayed, which are "o", "e", "o".
...
1.  *paste(collapse)* puts seperate elements in a vector together, and *''* is the interval, which is basically no interval here.
...
1.  Subsetting. Running *is.na(lord)* gives us *FALSE FALSE FALSE FALSE FALSE FALSE FALSE FALSE FALSE*;
